...
- ChatGPT via Codex: the model picker lists the models your Codex plan offers, and the `default` model now runs and displays Codex's current default model by name ("GPT 5.6 Sol (Codex default)" instead of "Codex default"), with the effort picker limited to the levels that model supports. The list comes from Codex's own local cache (`$CODEX_HOME/models_cache.json`, refreshed whenever the `codex` CLI runs); without it, behaviour is unchanged
