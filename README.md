...
**Documentation: [reyab.ca/projects/canvas-cli](https://reyab.ca/projects/canvas-cli)**
...
**After the answer.** Once a read has grounded the conversation, the tool memory adds an evidence checkpoint that lists what has been read and names the most likely next source, so the model keeps reading instead of settling early. When the model does answer, a recovery pass runs outside the tool-step budget: an answer that says something was not found triggers the tools it never tried (assignment list, announcement search, workspace and course search); an answer that quotes a date or figure from a search snippet without opening the document gets that document read and the answer regenerated on the full text (you'll see a "Checking the source" line first); a multi-part question with only one source read gets its second source. Verification then checks each date and figure against the evidence in context, so "Lab 4 is due March 20" is flagged when the schedule lists March 20 next to Lab 3 and March 27 next to Lab 4, and a "must" is flagged when the source only says "may".
